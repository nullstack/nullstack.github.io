  ...
  render() {
    ...
    return (
      <div class="w-full h-full fixed top-0 left-0 flex items-start justify-center z-50 blur-sm bg-gray-900 bg-opacity-80 px-4 py-8">
        <div class="w-full max-w-screen-md bg-gray-800 p-4 rounded-md shadow-md md:mt-48">
          <div class="flex justify-between mb-4">
            <span> {this.i18n.title} </span>
            <button class="text-gray-400" onclick={this.close}> {this.i18n.close} </button>
          </div>
          <input bind={this.term} oninput={this.search} type="search" class="text-white w-full bg-gray-900 p-4" placeholder={this.i18n.placeholder} />
          {this.results.length > 0 &&
            <ul class="mt-4 space-y-4 divide-y divide-gray-900">
